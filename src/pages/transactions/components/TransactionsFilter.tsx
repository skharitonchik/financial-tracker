...
export const TransactionsFilter: FC<TransactionsFilterProps> = ({
  ...
  }, [selectedUser, selectedCategory, dateTo, dateFrom, isNeedReload]);
      ...
      <Accordion sx={{ mb: 2 }}>
        <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="filterpanel-content" id="filterpanel-header">
          ...
          {Object.keys(filteredPlus).map((currencyItem, index) => {
            return (
              <Typography key={`${currencyItem}-${index}`} sx={{ ml: 3, color: '#008c7e', display: 'inline-flex' }}>
          ...
          {Object.keys(filteredMinus).map((currencyItem, index) => {
            return (
              <Typography key={`${currencyItem}-${index}`} sx={{ ml: 3, color: '#FF4842', display: 'inline-flex' }}>
        ...
        <AccordionDetails>
          ...
          <FormControl>
            <InputLabel id={'filter-user-select-label'}>User</InputLabel>
            <Select
              sx={{ minWidth: '120px' }}
              ...
              <MenuItem key={`filter-user-select-item-empty`} value={'Any'}>
                {'Any'}
              </MenuItem>
              {usersData.map((u, index) => (
                <MenuItem key={`filter-user-select-item-${index}`} value={u.name}>
                  {u.name}
                </MenuItem>
              ))}
            </Select>
          ...
          <FormControl sx={{ ml: 2 }}>
            <InputLabel id={'filter-category-select-label'}>Category</InputLabel>
            <Select
              sx={{ minWidth: '120px' }}
              ...
              onChange={(e: any) =>
                e.target.value !== 'Any' ? setSelectedCategory(e.target.value) : setSelectedCategory('')
              }>
              <MenuItem key={`filter-user-select-item-empty`} value={'Any'}>
                {'Any'}
              </MenuItem>
              {categoriesData.map((c, index) => (
                <MenuItem key={`filter-category-select-item-${index}`} value={c.id}>
                  {c.name}
                </MenuItem>
              ))}
            </Select>
